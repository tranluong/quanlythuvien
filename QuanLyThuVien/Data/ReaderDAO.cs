...
//Tìm độc giả
        ...
        {
            string strSQL = "SELECT dg.MaDG ,dg.TenDG ,dg.NgaySinh ,dg.NoiSinh,CASE dg.GioiTinh WHEN 1 THEN 'Nam' WHEN 0 THEN N'Nữ' END as 'Giới Tính',dg.DiaChi ,dg.SoDT,dg.TienDatCoc,dg.NgayDatCoc,nv.TenNV ,CASE dg.MaLoaiDG WHEN 1 THEN N'Sinh Vien' WHEN 2 THEN N'Giáo Viên'  END as 'Loại Độc Giả', dg.ThoiGianBatDau, dg.ThoiGianKetThuc   FROM tblDocGia dg join tblNhanVien nv on dg.MaNV=nv.MaNV order by dg.MaDG desc";
            DataTable dt = new DataTable();
            Database cls = new Database();
            try
            {
