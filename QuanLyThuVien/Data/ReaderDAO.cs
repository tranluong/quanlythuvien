...
//Tìm độc giả
        public DataTable SearchReader(int intType, string strKeyword, int intFilter)
        {
            ...
            if (intType == 0)//tìm theo Mã độc giả
                if (intFilter == 0)//Gần đúng
                    strSQL = "SELECT MaDG as 'Mã độc giả',TenDG as 'Độc giả' ,NgaySinh as 'Ngày sinh', NoiSinh as 'Nơi sinh', CASE GioiTinh WHEN 1 THEN 'Nam' WHEN 0 THEN N'Nữ' END as 'Giới tính',DiaChi as 'Địa chỉ' ,SoDT as 'Điện thoại',TienDatCoc as 'Tiền đặt cọc' ,NgayDatCoc as 'Ngày đặt cọc' ,CASE MaLoaiDG WHEN 1 THEN N'Sinh Vien' WHEN 2 THEN N'Giáo Viên'  END as 'Loại độc giả'  ,ThoiGianBatDau as 'Thời gian bắt đầu',ThoiGianKetThuc  as 'Thời gian kết thúc'  FROM tblDocGia WHERE MaDG like '%'+@Keyword+'%'";
                else//Chính xác
                    strSQL = "SELECT MaDG as 'Mã độc giả',TenDG as 'Độc giả' ,NgaySinh as 'Ngày sinh', NoiSinh as 'Nơi sinh', CASE GioiTinh WHEN 1 THEN 'Nam' WHEN 0 THEN N'Nữ' END as 'Giới tính',DiaChi as 'Địa chỉ' ,SoDT as 'Điện thoại' ,TienDatCoc as 'Tiền đặt cọc' ,NgayDatCoc as 'Ngày đặt cọc' ,CASE MaLoaiDG WHEN 1 THEN N'Sinh Vien' WHEN 2 THEN N'Giáo Viên'  END as 'Loại độc giả'  ,ThoiGianBatDau as 'Thời gian bắt đầu',ThoiGianKetThuc as 'Thời gian kết thúc'  FROM tblDocGia WHERE MaDG like @Keyword";
            else//tìm theo Tên độc giả
                if (intFilter == 0)//Gần đúng
                    strSQL = "SELECT MaDG as 'Mã độc giả',TenDG as 'Độc giả' ,NgaySinh as 'Ngày sinh', NoiSinh as 'Nơi sinh', CASE GioiTinh WHEN 1 THEN 'Nam' WHEN 0 THEN N'Nữ' END as 'Giới tính' ,DiaChi as 'Địa chỉ' ,SoDT as 'Điện thoại' ,TienDatCoc as 'Tiền đặt cọc',NgayDatCoc as 'Ngày đặt cọc' ,CASE MaLoaiDG WHEN 1 THEN N'Sinh Vien' WHEN 2 THEN N'Giáo Viên'  END as 'Loại độc giả'  ,ThoiGianBatDau as 'Thời gian bắt đầu'  , ThoiGianKetThuc as 'Thời gian kết thúc'  FROM tblDocGia where TenDG like '%'+@Keyword+'%'";
                else//Chính xác
                    strSQL = "SELECT MaDG as 'Mã độc giả',TenDG as 'Độc giả' ,NgaySinh as 'Ngày sinh', NoiSinh as 'Nơi sinh', CASE GioiTinh WHEN 1 THEN 'Nam' WHEN 0 THEN N'Nữ' END as 'Giới tính',DiaChi as 'Địa chỉ' ,SoDT as 'Điện thoại',TienDatCoc as 'Tiền đặt cọc' ,NgayDatCoc as 'Ngày đặt cọc' ,CASE MaLoaiDG WHEN 1 THEN N'Sinh Vien' WHEN 2 THEN N'Giáo Viên'  END as 'Loại độc giả'  ,ThoiGianBatDau as 'Thời gian bắt đầu'  , ThoiGianKetThuc as 'Thời gian kết thúc'  FROM tblDocGia where TenDG like @Keyword";
            SqlCommand cmd = new SqlCommand();
        ...
        {
            string strSQL = "SELECT dg.MaDG as 'Mã Độc Giả',dg.TenDG as 'Tên Độc Giả',dg.NgaySinh as 'Ngày Sinh' ,dg.NoiSinh as 'Nơi Sinh',CASE dg.GioiTinh WHEN 1 THEN 'Nam' WHEN 0 THEN N'Nữ' END as 'Giới Tính',dg.DiaChi as ' Địa Chỉ',dg.SoDT as 'Điện Thoại',dg.TienDatCoc as 'Tiền Đặt Cọc',dg.NgayDatCoc as 'Ngày Đặt Cọc',nv.TenNV as 'Họ và Tên' ,CASE dg.MaLoaiDG WHEN 1 THEN N'Sinh Vien' WHEN 2 THEN N'Giáo Viên' WHEN 8 THEN N'Bị Cấm' WHEN 9 THEN N'Chưa Đóng Phạt' END as 'Loại Độc Giả', dg.ThoiGianBatDau, dg.ThoiGianKetThuc   FROM tblDocGia dg join tblNhanVien nv on dg.MaNV=nv.MaNV order by dg.MaDG desc";
            DataTable dt = new DataTable();
            Database cls = new Database();
            try
            {
        ...
        public DataTable ComboboxLoaiDG()
        {
            string strSQL = "SELECT TOP 2 MaLoaiDG as 'Mã Loại Độc Giả', TenLoaiDG as 'Tên Loại Độc Giả' from tblLoaiDocGia";
            DataTable dt = new DataTable();
            Database cls = new Database();
            try
            {
