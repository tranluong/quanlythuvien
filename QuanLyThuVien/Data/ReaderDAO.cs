...
//Tìm độc giả
        ...
        {
            //string strSQL = "select MaDG as 'Mã Độc giả', TenDG as 'Tên Độc giả', GioiTinh as 'Giới tính', DaDatCoc as 'Đặt cọc', MaLoaiDG as 'Loại Độc giả' from tblDocgia order by MaDG Desc";
            string strSQL = "SELECT MaDG AS 'Mã độc giả',TenDG AS 'Tên độc giả',NgaySinh AS 'Ngày sinh',CASE GioiTinh WHEN 1 THEN 'Nam' WHEN 0 THEN N'Nữ' END AS 'Giới tính',CASE MaLoaiDG WHEN 0 THEN N'Sinh Vien,Giáo Viên'  END AS 'Loại độc giả',DiaChi AS 'Địa chỉ',SoDT AS 'Số điện thoại',NoiSinh AS 'Nơi sinh',,NgayDatCoc AS 'Ngày đặc cọc',(SELECT TenNV FROM tblNhanVien WHERE MaNV=MaNV) AS 'Người nhận tiền đặt cọc',(SELECT TenNV FROM tblNhanVien WHERE MaNV=NguoiCapNhat) AS 'Người cập nhật cuối',NgayCapNhat AS 'Ngày cập nhật cuối' FROM tblDocGia";
            DataTable dt = new DataTable();
            Database cls = new Database();
            try
            {
